<script setup>
  ...
  const actualizarEstado = ({id, estado}) => {
    ClienteService.cambiarEstado(id, {estado: !estado})
      // Se utiliza finIdex porque una cosa es el ID de la base de datos, pero otro es la posición en el arreglo de clientes
      .then(() => {
  ...
  const eliminarCliente = id => {
    ClienteService.eliminarCliente(id)
      .then(() => {
        // Accedemos a cada cliente y extraemos los clientes cuyo cliente ID sea diferente a el ID que le estamos pasando.
